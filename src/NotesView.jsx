...
/* Pocket motion — Notes */
.notesResponsivePage {
  animation: pocketNotesIn 320ms cubic-bezier(.22,1,.36,1) both;
}
.notesResponsivePage button,
.notesResponsivePage a {
  transition: transform 150ms cubic-bezier(.22,1,.36,1), background-color 150ms ease, border-color 150ms ease, color 150ms ease, box-shadow 180ms ease;
  -webkit-tap-highlight-color: transparent;
}
.notesResponsivePage button:not(:disabled):active {
  transform: translateY(1px) scale(.985);
}
.notesResponsivePage input,
.notesResponsivePage textarea,
.notesResponsivePage select {
  transition: border-color 150ms ease, box-shadow 180ms ease;
}
.notesResponsivePage input:focus,
.notesResponsivePage textarea:focus,
.notesResponsivePage select:focus {
  outline: none;
  border-color: #4FE36B !important;
  box-shadow: 0 0 0 3px rgba(79,227,107,.08);
}
@keyframes pocketNotesIn {
